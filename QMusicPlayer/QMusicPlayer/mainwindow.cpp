...
void MainWindow::songsListInit()                                        //歌曲列表初始化
{
    ...
    songList->setGeometry(0,
                          ...
                          (float)150/800*this->geometry().width(),
                          (float)350/480*this->geometry().height()
                          );//0,20,150,385设置歌曲列表的位置与大小
    ...
    for (int i = 0; i < mediaObjectInfo.count(); i++) {
        ...
        item->setSizeHint(QSize((float)150/800*this->geometry().width(),
...
void MainWindow::settingsListInit()
{
    ...
    settingsList->setGeometry(this->geometry().width()-80,this->geometry().height()/2-50,80,60);
...
void MainWindow::labelInit()
{
    ...
    labelSongsTitle->setText("世界很冷，有人在为你发热");
    ...
    labelArr[3]->setStyleSheet("QLabel{color:#FF7F50;}QLabel{background:transparent}");

    labelArr[3]->setText("You'll Never Walk Alone");                  //设置歌词文本初始显示内容
